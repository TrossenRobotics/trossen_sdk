...
- [x] Verified on rivet-01 (2026-08-13)
...
**rivet-01**, same kill test against the full config (4 arms + base + 3 ZEDs),
recovered with **no retry needed**: killed at 18:53:29, `camera_main opened` at
18:53:39. Bootstrap order is why — arms connect before cameras, and those ~10s
are enough for Argus to reap the dead client, so the race never opens. Worth
remembering when reproducing this: **a config that brings up arms first will
often not show the bug at all**.

So the retry was exercised deliberately instead, on the full rivet_01 config, by
holding `camera_main` from a separate process and releasing it mid-retry:

```
19:00:16  [ZED][ERROR] CAMERA STREAM FAILED TO START in sl::Camera::open()
19:00:16  camera_main (S/N 51287468) open failed: CAMERA STREAM FAILED TO START — retrying in 2s (1/4)
19:00:17  holder released
19:00:22  camera_main opened: 1920x1200 @ 30 FPS
19:00:32  all three open, session active
```

That is the incident's own error code, on the rig's own config, recovering.
Before this change the 19:00:16 line ended the session.
