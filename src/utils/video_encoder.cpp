 ...
 *
 * Stays silent for a context create() has muted via log_level_offset, i.e. a hardware candidate
 * being probed speculatively. Rejecting an option there just means that candidate is about to be
 * skipped, which is routine and not worth a line per camera per episode.
 *
...
std::unique_ptr<VideoEncoder> VideoEncoder::create(const Params& params) {
  // FFmpeg's default level prints a few lines of routine detail ("using cpu capabilities", the
  // chosen profile, the final ratefactor) to stderr for every encoder opened, which on a rig with
  // several cameras buries the tool's own output in what looks like error text. Warnings and
  // errors still come through. Only FFmpeg's untouched default is lowered, so an application that
  // deliberately set a level of its own keeps it.
  ...
  const std::vector<std::string> candidates = resolve_candidates(params);
  for (size_t i = 0; i < candidates.size(); ++i) {
    const std::string& name = candidates[i];
    // A hardware candidate that this machine cannot actually run is the expected case, not a
    ...
    // libavcodec logs a probe failure itself, at AV_LOG_ERROR, before avcodec_open2() returns:
    // an unusable nvenc prints "Cannot load libnvidia-encode.so.1" and friends on every single
    // create() call. log_level_offset is added to the level of each message this context emits,
    // so shifting ERROR up to DEBUG keeps those below the default AV_LOG_INFO threshold and out
    // of the terminal, while leaving the real failure of the last candidate visible.
    if (!is_last_candidate) {
      candidate_ctx->log_level_offset = AV_LOG_DEBUG - AV_LOG_ERROR;
    }
  ...
  std::cout << "  [ok] Video encoder '" << codec->name << "' (" << params.width << "x"
