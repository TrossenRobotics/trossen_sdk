...
void RealsenseCamera::find_cameras() {
    ...
    for (const auto& dev : devices) {
        ...
        if (!image_data.image.empty()) {
            ...
            spdlog::info("Enqueued image saving for camera: {} to file: {}", name, filename);
        } else {
...
trossen_ai_robot_devices::ImageData OpenCVCamera::read() {
    ...
    if (use_depth_) {
        spdlog::warn("Depth map requested but not supported in OpenCVCamera: {}", name_);
    }
    ...
    for (const std::string& path : targets_to_scan) {
        ...
        if (cap.isOpened()) {
            ...
            if (width > 0 && height > 0) {
                ...
                if (!frame.empty()) {
                    std::string filename = path.substr(path.find_last_of('/') + 1) + ".png";
