    ...
    if (config.camera_interface == "realsense") {
        for (const auto& cam_config : config.cameras) {
            cameras_.emplace_back(std::make_unique<trossen_ai_robot_devices::RealsenseCamera>(
                cam_config.name, cam_config.serial, cam_config.width, cam_config.height,
                cam_config.fps, cam_config.use_depth));
        }
        ...
        spdlog::error(
            "Unknown camera interface: {}. Supported interfaces are 'realsense' and 'opencv'.",
            config.camera_interface);
        throw std::runtime_error("Unknown camera interface: " + config.camera_interface);
    }
    spdlog::info("TrossenAIWidowXRobot initialized with name: {}, IP address: {}", name_,
                 ip_address_);
}
...
std::vector<trossen_ai_robot_devices::CameraType> TrossenAIWidowXRobot::get_camera_features()
    const {
...
TrossenAIBimanualWidowXRobot::TrossenAIBimanualWidowXRobot(
    ...
    right_robot_driver_ = std::make_unique<trossen_ai_robot_devices::TrossenAIArm>(
    ...
    spdlog::info(
        "TrossenAIBimanualWidowXRobot initialized with name: {}, Right IP address: {}, Left IP "
        "address: {}",
...
void TrossenAIBimanualWidowXRobot::get_observation(trossen_ai_robot_devices::State& state) {
    ...
    state.observation_state.insert(state.observation_state.end(), right_positions.begin(),
                                   right_positions.end());
    state.observation_state.insert(state.observation_state.end(), left_positions.begin(),
                                   left_positions.end());
    // Read camera images
    for (auto& camera : cameras_) {
        state.images.push_back(camera->async_read());
...
void TrossenAIBimanualWidowXRobot::send_action(const std::vector<double>& action) {
    ...
    if (action.size() <
        right_robot_driver_->get_num_joints() + left_robot_driver_->get_num_joints()) {
        spdlog::error("Error: Expected at least {} joint positions, got {}",
                      right_robot_driver_->get_num_joints() + left_robot_driver_->get_num_joints(),
                      action.size());
        return;
    }
    std::vector<double> right_action(action.begin(),
                                     action.begin() + right_robot_driver_->get_num_joints());
    std::vector<double> left_action(action.begin() + left_robot_driver_->get_num_joints(),
                                    action.end());
...
TrossenAIWidowXLeader::TrossenAIWidowXLeader(const trossen_sdk_config::WidowXLeaderConfig& config)
    ...
    spdlog::info("TrossenAIWidowXLeader initialized with name: {}, IP address: {}", name_,
                 ip_address_);
}
...
void TrossenAIWidowXLeader::configure() {
    robot_driver_->stage_arm();
    robot_driver_->write(trossen_sdk::EXTERNAL_EFFORT,
...
TrossenAIBimanualWidowXLeader::TrossenAIBimanualWidowXLeader(
    ...
    spdlog::info(
        "TrossenAIBimanualWidowXLeader initialized with name: {}, Right IP address: {}, Left IP "
        "address: {}",
        name_, right_ip_address_, left_ip_address_);
}
...
void TrossenAIBimanualWidowXLeader::configure() {
    ...
    right_robot_driver_->write(trossen_sdk::EXTERNAL_EFFORT,
                               std::vector<double>(right_robot_driver_->get_num_joints(), 0.0));
    left_robot_driver_->write(trossen_sdk::EXTERNAL_EFFORT,
