...
TrossenAIArm::TrossenAIArm(const std::string& name, const std::string& ip_address,
                           const std::string& model)
    : name_(name), ip_address_(ip_address), model_(model) {
    spdlog::info("TrossenAIArm initialized with name: {}, IP address: {}, model: {}", name_,
                 ip_address_, model_);
}
    ...
    try {
        if (model_ == trossen_sdk::LEADER_MODEL) {
            driver_.configure(trossen_arm::Model::wxai_v0,
                              trossen_arm::StandardEndEffector::wxai_v0_leader, ip_address_, true);
        } else if (model_ == trossen_sdk::FOLLOWER_MODEL) {
            driver_.configure(trossen_arm::Model::wxai_v0,
                              trossen_arm::StandardEndEffector::wxai_v0_follower, ip_address_,
                              true);
...
void TrossenAIArm::write(const std::string& data_name, const std::vector<double>& data) {
    ...
    if (data_name == trossen_sdk::POSITION) {
        ...
        if (!std::all_of(modes.begin(), modes.end(),
                         [](const auto& mode) { return mode == trossen_arm::Mode::position; })) {
            driver_.set_all_modes(trossen_arm::Mode::position);
        }
        // Check if the data size is correct for positions
        if (data.size() != driver_.get_num_joints()) {
            spdlog::error("Invalid data size for positions. Expected {} values.",
        ...
        const auto& modes = driver_.get_modes();
        bool all_velocity = std::all_of(modes.begin(), modes.end(), [](const auto& mode) {
            return mode == trossen_arm::Mode::velocity;
        });
        if (!all_velocity) {
        ...
        if (data.size() != driver_.get_num_joints()) {
            spdlog::error("Invalid data size for velocities. Expected {} values.",
                          driver_.get_num_joints());
            return;
        }
        ...
        const auto& modes = driver_.get_modes();
        bool all_efforts = std::all_of(modes.begin(), modes.end(), [](const auto& mode) {
            return mode == trossen_arm::Mode::external_effort;
        });
        if (!all_efforts) {
        ...
        if (data.size() != driver_.get_num_joints()) {
            spdlog::error("Invalid data size for external_efforts. Expected {} values.",
                          driver_.get_num_joints());
            return;
        }
