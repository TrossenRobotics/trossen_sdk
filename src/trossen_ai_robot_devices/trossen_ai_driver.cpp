...
TrossenAIArm::TrossenAIArm(const std::string& name,
                           const std::string& ip_address,
                           const std::string& model)
    : name_(name), ip_address_(ip_address), model_(model) {
  spdlog::info(
      "TrossenAIArm initialized with name: {}, IP address: {}, model: {}",
      name_, ip_address_, model_);
}
...
void TrossenAIArm::write(const std::string& data_name,
  ...
  if (data_name == trossen_sdk::POSITION) {
    ...
    if (!std::all_of(modes.begin(), modes.end(), [](const auto& mode) {
          return mode == trossen_arm::Mode::position;
        })) {
      driver_.set_all_modes(trossen_arm::Mode::position);
    }
    ...
    const auto& modes = driver_.get_modes();
    bool all_velocity = std::all_of(
        modes.begin(), modes.end(),
        [](const auto& mode) { return mode == trossen_arm::Mode::velocity; });
    if (!all_velocity) {
    ...
    if (data.size() != driver_.get_num_joints()) {
      spdlog::error(
          "Invalid data size for external_efforts. Expected {} values.",
          driver_.get_num_joints());
      return;
    }
...
void TrossenAIArm::stage_arm() {
  ...
  driver_.set_all_modes(trossen_arm::Mode::position);
  // Non-blocking call to move to the staged position this reduces the wait time
  // during connect and configure
