...
static inline std::string color_id(const std::string& cam) {
  return std::string("/cameras/") + cam + "/color/image";
}
static inline std::string depth_id(const std::string& cam) {
  return std::string("/cameras/") + cam + "/depth/image";
}
...
MockSyncedCameraProducer::MockSyncedCameraProducer(Config cfg) : cfg_(std::move(cfg)) {
  ...
  metadata_.description =
    "Produces synthetic synchronized color and depth frames for testing and diagnostics";
  metadata_.width = 640;
...
void MockSyncedCameraProducer::poll(
  const std::function<void(std::shared_ptr<data::RecordBase>)>& emit)
{
  ...
  if (warmup_remaining_ > 0) {
    ...
    last_emit_mono_ = now_ns;
     // discard both (color+depth) during warmup
    return;
  }
  ...
  // Emit color record
  {
    ...
    rec->encoding = cfg_.streams.color_encoding;
