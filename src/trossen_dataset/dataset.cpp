...
void EpisodeData::clear() { buffer_.clear(); }

TrossenAIDataset::TrossenAIDataset(
    const std::string &dataset_name, const std::string &task_name,
    const std::shared_ptr<trossen_ai_robot_devices::robot::TrossenRobot> &robot,
    std::filesystem::path root, std::string repo_id, bool run_compute_stats,
    bool overwrite, int num_image_writer_threads_per_camera, double fps)
    : dataset_name_(dataset_name),
      task_name_(task_name),
      robot_(robot),
      root_(root),
      repo_id_(repo_id),
      run_compute_stats_(run_compute_stats),
      overwrite_(overwrite),
      fps_(fps) {
  spdlog::info("TrossenAIDataset : {}", dataset_name_);

  // Create the dataset directory structure using root, repo_id, and
  // dataset_name
  ...
  if (std::filesystem::exists(dataset_dir)) {
    ...
    if (overwrite_) {
      ...
      std::filesystem::create_directories(dataset_dir /
                                          trossen_sdk::METADATA_DIR);
      std::filesystem::create_directories(dataset_dir / trossen_sdk::VIDEO_DIR /
                                          "chunk-000");
      std::filesystem::create_directories(dataset_dir /
      ...
      if (!verify()) {
        throw std::runtime_error(
            "Dataset verification failed: metadata does not match or is "
            "incomplete.");
      }
...
void TrossenAIDataset::add_frame(FrameData *frame) {
  ...
  for (const auto &image_data : frame->images) {
    ...
    image_writer_.push(trossen_ai_robot_devices::ImageSaveTask{
        image_data.image, image_file_path});
    ...
    if (!image_data.depth_map.empty()) {
      ...
      image_writer_.push(trossen_ai_robot_devices::ImageSaveTask{
          image_data.depth_map, depth_file_path});
    }
  }
}
...
void TrossenAIDataset::save_episode() {
  ...
  arrow::ListBuilder observation_builder(
  ...
  auto *observation_value_builder =
      static_cast<arrow::DoubleBuilder *>(observation_builder.value_builder());
  auto *action_value_builder =
      static_cast<arrow::DoubleBuilder *>(action_builder.value_builder());
  ...
  for (const auto &sample : current_episode_->get_frames()) {
    ...
    check_status(timestamp_builder.Append(sample.timestamp_s),
                 "Failed to append timestamp: {}");

    check_status(observation_builder.Append(),
                 "Failed to append observation list: {}");
    for (const auto &pos : sample.observation_state)
      check_status(observation_value_builder->Append(pos),
                   "Failed to append observation state value: {}");

    check_status(action_builder.Append(), "Failed to append action list: {}");
    for (const auto &act : sample.action)
      check_status(action_value_builder->Append(act),
                   "Failed to append action value: {}");

    check_status(episode_idx_builder.Append(sample.episode_idx),
  ...
  auto finish_builder = [](auto &builder, std::shared_ptr<arrow::Array> &array,
    ...
    if (!status.ok()) {
      spdlog::error("[Arrow Error] Failed to finish {} builder: {}", name,
                    status.ToString());
      return false;
    }
    return true;
  ...
  if (!finish_builder(observation_builder, observation_array, "observation"))
    return;
  if (!finish_builder(action_builder, action_array, "action")) return;
  if (!finish_builder(episode_idx_builder, episode_idx_array, "episode index"))
    return;
  if (!finish_builder(frame_idx_builder, frame_idx_array, "frame index"))
    return;
  if (!finish_builder(index_builder, index_array, "index")) return;
  if (!finish_builder(task_index_builder, task_index_array, "task index"))
    return;
  ...
  std::error_code ec;
  std::filesystem::create_directories(
      std::filesystem::path(output_path_).parent_path(), ec);
  if (ec) {
    spdlog::error("[Filesystem Error] Failed to create directories: {}",
                  ec.message());
    return;
  }
  ...
  if (!result.ok()) {
    spdlog::error("[Arrow Error] Failed to open file output stream: {}",
  ...
  auto status = parquet::arrow::WriteTable(*table, arrow::default_memory_pool(),
                                           outfile, 1024);
  if (!status.ok()) {
    spdlog::error("[Parquet Error] Failed to write table: {}",
                  status.ToString());
  } else {
...
nlohmann::json TrossenAIDataset::compute_list_stats(
...
std::vector<int> TrossenAIDataset::sample_indices(int dataset_len,
                                                  int min_samples,
                                                  int max_samples,
                                                  float power) {
...
cv::Mat TrossenAIDataset::auto_downsample(const cv::Mat &img, int target_size,
                                          int max_threshold) {
  int h = img.rows;
  int w = img.cols;
...
std::vector<cv::Mat> TrossenAIDataset::sample_images(
...
FeatureStats TrossenAIDataset::compute_image_stats(
...
nlohmann::json TrossenAIDataset::convert_stats_to_json(
...
void TrossenAIDataset::compute_statistics(std::shared_ptr<arrow::Table> table,
                                          int episode_index) {
  ...
  for (const auto &camera_info : robot_->get_camera_features()) {
    ...
    std::filesystem::path episode_image_dir =
        std::filesystem::path(image_folder_path) / "chunk-000" /
        camera_info.name / episode_folder_name;
    if (!std::filesystem::exists(episode_image_dir)) {
      spdlog::warn("Image directory does not exist: {}",
                   episode_image_dir.string());
      continue;
    }
...
void TrossenAIDataset::convert_to_videos() const {
  ...
  std::string images_path =
      metadata_->get_info_entry("image_path") + "/chunk-000";
  ...
  for (const auto &cam_dir : std::filesystem::directory_iterator(images_path)) {
    ...
    for (const auto &episode_dir :
         std::filesystem::directory_iterator(cam_dir.path())) {
      ...
      threads.emplace_back([=, &log_mutex, &active_threads]() {
        try {
          ...
          for (const auto &file :
               std::filesystem::directory_iterator(episode_dir.path())) {
          ...
          // numbering e.g. image_0.jpg, image_1.jpg, ..., image_N.jpg This
          // allows FFmpeg to read the images in order without the need for
          // sorting
          ...
          //  speed/quality tradeoff -pix_fmt yuv420p for compatibility Redirect
          //  output to /dev/null to suppress console output
          ...
          {
            std::lock_guard<std::mutex> lock(log_mutex);
            spdlog::debug("FFmpeg command for {} took {} ms", episode_name,
                          duration_ms);
          }
          std::lock_guard<std::mutex> lock(log_mutex);
          if (ret_code != 0) {
...
int TrossenAIDataset::get_num_existing_episodes() const {
  ...
  std::string data_path =
      root_.string() + "/" + repo_id_ + "/" + dataset_name_ + "/data/chunk-000";
...
std::vector<std::vector<double>> TrossenAIDataset::read(
    int episode_index) const {
  std::filesystem::path dataset_path =
      root_ / repo_id_ / dataset_name_ / trossen_sdk::DATA_PATH_DIR /
      ("episode_" + std::to_string(episode_index) + ".parquet");
  ...
  oss << "data/chunk-" << std::setw(3) << std::setfill('0') << chunk_index
      << "/episode_" << std::setw(6) << std::setfill('0')
      << episode_idx_in_chunk << ".parquet";
  std::string output_file =
      (root_ / repo_id_ / dataset_name_ / oss.str()).string();

  // Open the file and handle errors
  auto infile_result = arrow::io::ReadableFile::Open(output_file);
  if (!infile_result.ok()) {
    spdlog::error("Failed to open Arrow file: {}",
  ...
  auto values_array =
      std::static_pointer_cast<arrow::DoubleArray>(joints_array->values());
...
Metadata::Metadata(const std::string &dataset_name, const std::string &repo_id,
  ...
  if (existing) {
    ...
    set_info_entry("image_path",
                   (base_path / trossen_sdk::IMAGES_DIR).string());
    save_all();
  }
}
...
void Metadata::update_info(int additional_frames) {
  ...
  int total_frames =
      info_.contains("total_frames") ? info_["total_frames"].get<int>() : 0;
  ...
  const std::string output_path =
      root_.string() + "/" + repo_id_ + "/" + dataset_name_ + "/README.md";
...
void Metadata::remove_info_entry(const std::string &key) { info_.erase(key); }

void Metadata::clear_info() { info_.clear(); }
...
void Metadata::save_jsonl_file(const std::string &path,
                               const std::vector<nlohmann::json> &data) const {
...
void Metadata::save_all() const {
  ...
  save_info_file();
  save_jsonl_file((meta_path / trossen_sdk::JSONL_EPISODES).string(),
                  episode_data_);
  save_jsonl_file((meta_path / trossen_sdk::JSONL_EPISODE_STATS).string(),
                  episode_stats_data_);
...
std::string Metadata::generate_readme(const nlohmann::json &info_json) const {
  ...
  out << "This dataset was created using "
