...
EpisodeData::EpisodeData(int64_t episode_idx) : episode_idx_(episode_idx) {
    ...
    buffer_.reserve(
        100);  // Reserve space for 100 frames initially can be adjusted based on the episode length
}
...
TrossenAIDataset::TrossenAIDataset(
    ...
    std::filesystem::path root, std::string repo_id, bool run_compute_stats, bool overwrite,
    int num_image_writer_threads_per_camera, double fps)
    ...
    if (std::filesystem::exists(dataset_dir)) {
        ...
        if (overwrite_) {
            ...
            std::filesystem::create_directories(dataset_dir / trossen_sdk::DATA_PATH_DIR /
                                                "chunk-000");
            ...
            metadata_ =
                std::make_unique<Metadata>(dataset_name_, repo_id_, task_name_, root_, false);
        }
        // If overwrite is false, load existing metadata and verify the dataset
        else {
            metadata_ =
                std::make_unique<Metadata>(dataset_name_, repo_id_, task_name_, root_, true);
            if (!verify()) {
                throw std::runtime_error(
                    "Dataset verification failed: metadata does not match or is incomplete.");
            }
...
void TrossenAIDataset::add_frame(FrameData &frame) {
    ...
    // Push images to the image writer for asynchronous writing with appropriate filenames using
    // frame index
    for (const auto &image_data : frame.images) {
        const std::string &camera_name = image_data.camera_name;
        std::string image_path = fmt::format(trossen_sdk::IMAGE_PATH, 0, camera_name,
        ...
        image_writer_.push(
            trossen_ai_robot_devices::ImageSaveTask{image_data.image, image_file_path});
        ...
        if (!image_data.depth_map.empty()) {
            ...
            image_writer_.push(
                trossen_ai_robot_devices::ImageSaveTask{image_data.depth_map, depth_file_path});
        }
    }
}
...
void TrossenAIDataset::save_episode() {
    ...
    auto table = arrow::Table::Make(
        schema, {timestamp_array, observation_array, action_array, episode_idx_array,
    ...
    if (run_compute_stats_) compute_statistics(table, current_episode_->get_episode_idx());
...
nlohmann::json TrossenAIDataset::compute_list_stats(
    ...
    for (int d = 0; d < dim; ++d) {  // Compute mean and standard deviation for each dimension
...
bool TrossenAIDataset::verify() const {
    ...
    if (metadata_->get_info_entry("dataset_name") != dataset_name_ ||
        ...
        spdlog::error(
            "Dataset metadata does not match the dataset name, robot name, or task name.");
        return false;
    }
    return true;
}

std::vector<int> TrossenAIDataset::sample_indices(int dataset_len, int min_samples, int max_samples,
                                                  float power) {
    // Calculate the number of samples based on the power law
    // Clamp the number of samples between min_samples and max_samples
    int num_samples =
        std::clamp(static_cast<int>(std::pow(dataset_len, power)), min_samples, max_samples);
...
cv::Mat TrossenAIDataset::auto_downsample(const cv::Mat &img, int target_size, int max_threshold) {
    ...
    float factor =
        (w > h) ? (w / static_cast<float>(target_size)) : (h / static_cast<float>(target_size));
...
FeatureStats TrossenAIDataset::compute_image_stats(const std::vector<cv::Mat> &images) {
    ...
    for (const auto &img : images) {
        ...
        for (int c = 0; c < num_channels; ++c) {
            ...
            channel_values[c].insert(channel_values[c].end(), (float *)channels[c].datastart,
...
void TrossenAIDataset::compute_statistics(std::shared_ptr<arrow::Table> table, int episode_index) {
    ...
    for (const auto &camera_info : robot_->get_camera_features()) {
        ...
        std::filesystem::path episode_image_dir = std::filesystem::path(image_folder_path) /
                                                  "chunk-000" / camera_info.name /
                                                  episode_folder_name;
        ...
        for (const auto &episode_dir : std::filesystem::directory_iterator(cam_dir.path())) {
            ...
            threads.emplace_back([=, &log_mutex]() {
                try {
                    ...
                    //  speed/quality tradeoff -pix_fmt yuv420p for compatibility Redirect output to
                    //  /dev/null to suppress console output
                    std::ostringstream ffmpeg_cmd;
                    ffmpeg_cmd << "ffmpeg -y -framerate " << fps << " -i " << input_pattern.string()
...
std::vector<std::vector<double>> TrossenAIDataset::read(int episode_index) const {
    std::filesystem::path dataset_path = root_ / repo_id_ / dataset_name_ /
                                         trossen_sdk::DATA_PATH_DIR /
    ...
    oss << "data/chunk-" << std::setw(3) << std::setfill('0') << chunk_index << "/episode_"
        << std::setw(6) << std::setfill('0') << episode_idx_in_chunk << ".parquet";
    ...
    auto values_array = std::static_pointer_cast<arrow::DoubleArray>(joints_array->values());
...
Metadata::Metadata(const std::string &dataset_name, const std::string &repo_id,
                   const std::string &task_name, std::filesystem::path root, bool existing)
    : dataset_name_(dataset_name),
      repo_id_(repo_id),
      task_name_(task_name),
      root_(std::move(root)) {
...
void Metadata::add_features(const trossen_ai_robot_devices::robot::TrossenRobot &robot) {
    ...
    camera_feature["info"] = {{"video.fps", 30.0},           {"video.height", 480},
