...
class RunnerOutcome:
    ...
    """The caller's cancel event fired before the child finished.

    Distinct from `timed_out` on purpose: a cancelled run proves nothing about
    the hardware, so a caller must not report it as a failure. Terminating the
    child routinely makes it log on its way down, which is why this is checked
    ahead of `failure_marker()`.
    """
...
async def stream_runner(
    ...
    Setting `cancel` stops the run and sets `outcome.cancelled`. It is honoured
    even while the child is silent — the read is raced against the event rather
    than polled between lines, because the moment somebody reaches for Cancel is
    exactly the moment a runner is blocked on an unreachable arm's TCP connect
    and producing no output at all.

    The child is always reaped: on cancel, on timeout, on an exception in the
    consumer of this generator, and on normal completion.
    """
    try:
    ...
    # One long-lived waiter reused across iterations rather than a fresh task per
    # line: this loop runs once per output line, and an event that is already set
    # stays set, so re-awaiting it costs nothing. Created before the try because
    # the finally cancels it.
    ...
    try:
        ...
        while True:
            if cancel is not None and cancel.is_set():
                # Checked before the first read too, so a cancel that arrives
                # while the child is still launching is not missed.
            ...
            if cancel_wait is None:
                ...
                # `wait_for` on the read alone cannot be interrupted, so race the
                # two explicitly and drop whichever loses.
