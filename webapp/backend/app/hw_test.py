...
async def stream_system_hardware_test(
    ...
    # Registered before the runner is launched so a Cancel arriving during launch
    # is still honoured — `stream_runner` checks the event before its first read.
    ...
    try:
        ...
        # everything yielded here is genuine SDK output — the frontend never sees
        # the internal sentinels.
        async for line in runner_proto.stream_runner(
            "app.hw_test_runner", system.config, timeout_s, outcome, cancel_event
        ):
            yield _sse("progress", message=line)
    finally:
        # Released before anything below can return, so a second Cancel arriving
        # while this run is already finishing gets an honest 404 rather than
        # setting an event nobody will read.
        _release_cancel_slot()

    # Checked FIRST, ahead of the timeout and the failure-marker scan. Terminating
    # the runner mid-bring-up routinely makes it log `[error]` on its way down, and
    # a test the operator stopped on purpose must not be reported as hardware that
    # failed — that would stamp the badge red and send them debugging a fault that
    # does not exist.
