...
struct ProducerConfig {
    std::string stream_id;  // Producer ID (loaded from JSON 'id' field)
    std::string type;  // Producer type
    std::string leader_name;  // For teleop producers
    std::string follower_name;  // For teleop producers
    std::string camera_name;  // For camera producers
    std::string serial_number;  // For RealSense cameras
    int device_index;  // For OpenCV cameras
    int width;
    int height;
    int fps;
    std::string encoding;
