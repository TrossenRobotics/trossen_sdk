 ...
 * This function reads the JSON file, determines the type of leader robot
 * configuration, and deserializes it into the appropriate derived class of
 * LeaderConfigBase.
 */
 ...
 * This function reads the JSON file, determines the type of follower robot
 * configuration, and deserializes it into the appropriate derived class of
 * RobotConfigBase.
 */
 ...
 * This function creates an instance of the appropriate TrossenRobot derived
 * class based on the type of the provided RobotConfigBase object.
 */
inline std::shared_ptr<trossen_ai_robot_devices::robot::TrossenRobot>
create_robot_from_config(const trossen_sdk_config::RobotConfigBase& config) {
  // Determine the type of robot and create the appropriate instance
  if (config.get_name() == "widowxai") {
    auto leader_config =
        dynamic_cast<const trossen_sdk_config::WidowXRobotConfig&>(config);
    return std::make_shared<
        trossen_ai_robot_devices::robot::TrossenAIWidowXRobot>(leader_config);
  } else if (config.get_name() == "bimanual_widowxai") {
    auto robot_config =
        dynamic_cast<const trossen_sdk_config::BimanualWidowXRobotConfig&>(
            config);
    return std::make_shared<
        trossen_ai_robot_devices::robot::TrossenAIBimanualWidowXRobot>(
        robot_config);
  } else {
 ...
 * This function creates an instance of the appropriate TrossenLeader derived
 * class based on the type of the provided LeaderConfigBase object.
 */
...
create_leader_from_config(const trossen_sdk_config::LeaderConfigBase& config) {
  ...
  if (config.get_name() == "widowxai_leader") {
    auto leader_config =
        dynamic_cast<const trossen_sdk_config::WidowXLeaderConfig&>(config);
    return std::make_shared<
        trossen_ai_robot_devices::teleoperator::TrossenAIWidowXLeader>(
        leader_config);
  } else if (config.get_name() == "bimanual_widowxai_leader") {
    auto leader_config =
        dynamic_cast<const trossen_sdk_config::BimanualWidowXLeaderConfig&>(
            config);
    return std::make_shared<
        trossen_ai_robot_devices::teleoperator::TrossenAIBimanualWidowXLeader>(
        leader_config);
  } else {
