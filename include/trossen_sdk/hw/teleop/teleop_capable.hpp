...
// (turning LEFT), lift up, strafe left.
//
// A raw joystick axis carries no convention of its own — libtrossen_arm
// documents its handle stick only as "0 to 4095" — so which raw axis and which
// sign feeds each axis here is a per-device MEASUREMENT, not something to derive.
// See the measured Glide handle geometry in glide_base_component.hpp. Getting it
// wrong is invisible in code review and reads on hardware as "rotate right turns
// it left" or "forward drives it sideways".
namespace base_axis {
