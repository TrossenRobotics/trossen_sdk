 ...
 * This structure holds the timestamp, observation state, action, and other relevant information for
 * a single frame
 */
struct FrameData {
 ...
 * This class handles loading, saving, and updating metadata information such as dataset info,
 * episodes, tasks, and statistics It uses JSON format for storing metadata and provides methods to
 * manipulate the data
 */
class Metadata {
   public:
...
class TrossenAIDataset {
   public:
    ...
    explicit TrossenAIDataset(
        ...
        std::filesystem::path root, std::string repo_id, bool run_compute_stats = true,
        bool overwrite = false, int num_image_writer_threads_per_camera = 4, double fps = 30.0);
     ...
     */
    std::string get_image_path() const { return metadata_->get_info_entry("image_path"); }

    /**
     * @brief Get the video path for the dataset
     * @return Video path as a string
     */
    std::string get_videos_path() const { return metadata_->get_info_entry("videos_path"); }
    ...
    /**
     * @brief Static method to read a Parquet file and return its contents as a vector of vectors of
     * doubles
     ...
     */
    std::vector<int> sample_indices(int dataset_len, int min_samples = 100, int max_samples = 10000,
                                    float power = 0.75f);
