 ...
 * This structure holds the min, max, mean, and standard deviation values for a
 * specific feature
 */
struct FeatureStats {
 ...
 * This structure holds the timestamp, observation state, action, and other
 * relevant information for a single frame
 */
struct FrameData {
 ...
 * This class handles loading, saving, and updating metadata information such as
 * dataset info, episodes, tasks, and statistics It uses JSON format for storing
 * metadata and provides methods to manipulate the data
 */
class Metadata {
 public:
  /**
   ...
   */
  explicit Metadata(const std::string &dataset_name, const std::string &repo_id,
 ...
 private:
   ...
   */
  void save_jsonl_file(const std::string &path,
                       const std::vector<nlohmann::json> &data) const;
};

/**
 * @brief Class to manage data for a single episode
 * This class handles adding frames to the episode, closing the episode, and
 * retrieving frames It maintains a buffer of frames and the episode index
 */
class EpisodeData {
 public:
  /**
 ...
 * This class handles adding frames, saving episodes, verifying the dataset
 * structure, computing statistics, and converting images to videos It maintains
 * metadata, a buffer of episodes, and an image writer for asynchronous image
 * saving
 */
class TrossenAIDataset {
 public:
   ...
   * @param num_image_writer_threads_per_camera Number of threads for image
   * writing per camera
   * @param fps Frames per second for video conversion
   * Initializes the dataset with the provided parameters and sets up metadata
   * and image writer If the dataset already exists and overwrite is false, it
   * will load existing metadata
   */
  explicit TrossenAIDataset(
      const std::string &name, const std::string &task_name,
      const std::shared_ptr<trossen_ai_robot_devices::robot::TrossenRobot>
          &robot,
      std::filesystem::path root, std::string repo_id,
      bool run_compute_stats = true, bool overwrite = false,
      int num_image_writer_threads_per_camera = 4, double fps = 30.0);

  /**
   * @brief Verify the dataset structure and metadata
   * @return True if the dataset structure and metadata are valid, false
   * otherwise
   */
  bool verify() const;
   ...
   */
  size_t get_num_episodes() const { return episodes_buffer_.size(); }
   ...
   * This function processes the images saved in the dataset and converts them
   * into video files
   */
  ...
  /**
   * @brief Static method to read a Parquet file and return its contents as a
   * vector of vectors of doubles
   * @param output_file Path to the Parquet file
   * @return Vector of vectors of doubles representing the data in the Parquet
   * file
   */
   ...
   */
  nlohmann::json compute_list_stats(
   ...
   * @param episode_index Index of the episode for which statistics are being
   * computed
   */
  void compute_statistics(std::shared_ptr<arrow::Table> table,
                          int episode_index);
