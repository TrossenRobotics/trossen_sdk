...
def test_record_types():
    ...
    tj = ts.TeleopJointStateRecord(
        ts=now, seq=0, id="teleop",
        actions=[1.0], observations=[2.0]
    )
    assert tj.actions[0] == 1.0
...
def test_producer_metadata():
    """ProducerMetadata can be constructed."""
    import trossen_sdk as ts

    meta = ts.ProducerMetadata()
    meta.type = "test"
    meta.id = "test_0"
