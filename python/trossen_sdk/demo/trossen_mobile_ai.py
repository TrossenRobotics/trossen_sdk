...
# Python equivalent of examples/trossen_mobile_ai/trossen_mobile_ai.cpp.
#
# Hardware setup:
...
#
# Usage:
#   python -m trossen_sdk.demo.trossen_mobile_ai
#   python -m trossen_sdk.demo.trossen_mobile_ai --config path/to/config.json
...
def main():
    parser = argparse.ArgumentParser(
        description="Trossen Mobile AI Data Collection")
    parser.add_argument(
        "--config",
    ...
    if cfg.hardware.mobile_base is not None:
        config_lines.append("Mobile base:          SLATE")
    config_lines.append(
        f"Teleop:               {'enabled' if cfg.teleop.enabled else 'disabled'} "
        f"({len(cfg.teleop.pairs)} pairs)")
    ts.print_config_banner("Trossen Mobile AI Kit (Python)", config_lines)
    ...
    base_component = None
    if cfg.hardware.mobile_base is not None:
        base_component = ts.HardwareRegistry.create(
            "slate_base", "slate_base", cfg.hardware.mobile_base.to_json())
        print(f"  [ok] Mobile base (SLATE)")
    ...
    for prod_cfg in cfg.producers:
        ...
        elif prod_cfg.type == "slate_base" and base_component:
            prod = ts.ProducerRegistry.create(
                "slate_base", base_component, prod_cfg.to_registry_json())
            mgr.add_producer(prod, period_ms)
            print(f"  [ok] Base producer [{prod_cfg.stream_id}] "
                  f"({prod_cfg.poll_rate_hz} Hz)")
    ...
    streams = f"{len(cfg.hardware.arms)} arms + {len(cfg.hardware.cameras)} cameras"
    if base_component:
        streams += " + mobile base"
    ts.print_final_summary(
        stats.total_episodes_completed, root, [f"Data streams: {streams}"])
