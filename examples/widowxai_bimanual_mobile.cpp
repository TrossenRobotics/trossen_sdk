/**
 * @file widowxai_bimanual_mobile.cpp
 * @brief Complete Bimanual AI Kit demo with all 4 arms and SLATE mobile base
 *
 * This demo records all 4 arms (both leaders and followers) along with SLATE mobile base velocities:
 * - 4 Trossen AI arms (all recorded: 2 leaders + 2 followers)
 * - 1 SLATE mobile base (velocity data recorded)
 ...
 *   Mobile Base:    SLATE base (velocity data recorded)
 *
 * Usage:
 *   ./widowxai_bimanual_mobile
 */
...
int main(int argc, char** argv) {
  ...
  // Create SLATE mobile base via registry
  nlohmann::json slate_base_hw_cfg = {
    {"reset_odometry", false},
    {"enable_torque", false}
  };
  slate_base_component = std::make_shared<trossen::hw::base::SlateBaseComponent>("slate_base");
  slate_base_component->configure(slate_base_hw_cfg);
  ...
  auto base_prod = trossen::runtime::ProducerRegistry::create(
    "slate_base", slate_base_component, base_prod_cfg);
  ...
  while (true) {
    ...
    // Stop episode and wait for threads
    ...
    if (velocity_logging_thread.joinable()) {
      velocity_logging_thread.join();
    }
  ...
  std::vector<std::string> extra_info = {
    "Data streams:         4 arms (all recorded) + mobile base velocity"
  };
