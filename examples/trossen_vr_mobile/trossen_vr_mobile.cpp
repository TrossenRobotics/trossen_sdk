 ...
 * @brief VR-driven bimanual mobile demo — 2 follower arms + SLATE base + 3 cameras.
 *
 * Extends `trossen_vr_stationary` with a mobile base, all driven from a single
 * VR headset:
 *   - Left  VR controller  → follower_left arm  (cartesian pose + gripper).
 *   - Right VR controller  → follower_right arm (cartesian pose + gripper).
 *   - VR thumbsticks        → SLATE base (right stick = forward/back,
 *                             left stick = yaw).
 *   - Right A/B buttons     → session control (start/advance, re-record).
 *   - Left  X/Y buttons     → session control (stop-early, stop-session).
 *
 * The hand-grip (hand trigger) is the deadman switch per arm — the arm mirrors only
 * while the hand-grip is held, and re-gripping re-anchors the offset with no jump.
 *
 * All VR components share one process-wide VrSession (one network connection)
 * and claim non-overlapping inputs via VrSession::claim_inputs(), so
...
int main(int argc, char** argv) {
  ...
  // carry VR hardware). Copy it out for the VR component setup below;
  // SdkConfig ignores the unknown `vr` key when parsing `j`.
  ...
  if (vr_cfg.contains("base_controllers")) {
    for (const auto& [id, entry] : vr_cfg["base_controllers"].items()) {
      config_lines.push_back(
        "VR base [" + id + "]: linear=" +
        entry.value("linear_controller_type", std::string{"right"}) +
        " angular=" +
        entry.value("angular_controller_type", std::string{"left"}) +
        " thumbstick, port " +
        std::to_string(entry.value("vr_port", 9000)));
    }
  }
  ...
  if (cfg.hardware.mobile_base.has_value()) {
    // Construct + register under "slate_base" so the teleop pair config
    // resolves the follower by that identifier.
  ...
  // ── Initialize VR leaders (bind the shared network port) ────────────────

  std::vector<std::shared_ptr<trossen::hw::vr::VrArmComponent>>
    vr_arm_components;
  if (vr_cfg.contains("arm_controllers")) {
    for (const auto& [id, entry] : vr_cfg["arm_controllers"].items()) {
      auto component = trossen::hw::HardwareRegistry::create(
  ...
  std::vector<std::shared_ptr<trossen::hw::vr::VrBaseComponent>>
    vr_base_components;
  if (vr_cfg.contains("base_controllers")) {
    for (const auto& [id, entry] : vr_cfg["base_controllers"].items()) {
      auto component = trossen::hw::HardwareRegistry::create(
        "vr_base_component", id, entry, true);
      vr_base_components.push_back(
        std::dynamic_pointer_cast<trossen::hw::vr::VrBaseComponent>(component));
      std::cout << "  [ok] VR base component [" << id << "] (linear="
                << entry.value("linear_controller_type", std::string{"right"})
                << " angular="
                << entry.value("angular_controller_type", std::string{"left"})
                << " thumbstick)\n";
  ...
  // ── Teleop controllers ──────────────────────────────────────────────────
  // VrArmComponent / VrBaseComponent registered above are found by the factory
  // via ActiveHardwareRegistry, paired with the followers + base, and wrapped
  // in TeleopControllers. add_teleop() hands lifecycle ownership to
  // SessionManager (per-episode prepare/start/reset and shutdown).
  ...
  // Every session control component drives the same SessionManager primitives,
  // exactly mirroring what the keyboard does inside monitor_episode() /
  // wait_for_reset(). Keyboard and VR buttons are therefore interchangeable.
  ...
  std::cout << "\nReady. Press A on the right VR controller (or ENTER) to start.\n"
            << "  Hold each hand-grip trigger to teleop the matching arm.\n"
            << "  Push the thumbsticks to drive the base "
               "(right = forward/back, left = turn).\n"
  ...
  std::vector<std::string> extra_info = {
    "Data streams:         " +
      ...
      std::to_string(vr_base_components.size()) + " VR base controller(s)"
  };
